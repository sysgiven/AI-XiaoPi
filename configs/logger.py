...
from configs.settings import check_config_file
...
def setup_logging(check_full_config=True):
    """从配置文件中读取日志配置，并设置日志输出格式和级别

    Args:
        check_full_config: 是否检查完整配置文件（data/.config.yaml），弹幕模式下可设为 False
    """
    ...
    try:
        ...
        config = {
            "log": {
                ...
                "log_format_file": "{time:YYYY-MM-DD HH:mm:ss} - {name} - {level} - {extra[tag]} - {message}",
                "log_level": "INFO",
                "log_dir": "tmp",
                "log_file": "server.log",
                "data_dir": "data",
