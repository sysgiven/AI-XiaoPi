...
def initialize_modules(
    ...
    # 初始化TTS模块
    if init_tts:
        if not config.get("selected_module"):
            raise ValueError("配置文件中缺少 'selected_module' 字段，请检查配置文件格式")
        if not config["selected_module"].get("TTS"):
            raise ValueError("配置文件中缺少 'selected_module.TTS' 字段")
    ...
    if init_llm:
        if not config.get("selected_module"):
            raise ValueError("配置文件中缺少 'selected_module' 字段，请检查配置文件格式")
        if not config["selected_module"].get("LLM"):
            raise ValueError("配置文件中缺少 'selected_module.LLM' 字段")

        select_llm_module = config["selected_module"]["LLM"]
        llm_config = config.get("LLM", {}).get(select_llm_module)
        if not llm_config:
            raise ValueError(f"配置文件中缺少 'LLM.{select_llm_module}' 字段")

        llm_type = (
            select_llm_module
            if "type" not in llm_config
            else llm_config["type"]
        )
...
def initialize_tts(config):
    # 添加配置检查
    if not config.get("selected_module"):
        raise ValueError("配置文件中缺少 'selected_module' 字段")
    if not config["selected_module"].get("TTS"):
        raise ValueError("配置文件中缺少 'selected_module.TTS' 字段")
    ...
    if not config.get("TTS"):
        raise ValueError("配置文件中缺少 'TTS' 字段")
    if not config["TTS"].get(select_tts_module):
        raise ValueError(f"配置文件中缺少 'TTS.{select_tts_module}' 字段")

    tts_config = config["TTS"][select_tts_module]
    tts_type = (
        select_tts_module
        if not tts_config or "type" not in tts_config
